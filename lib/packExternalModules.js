 ...
 * @this - The active plugin instance
 */
function getProdModules(externalModules, packagePath, dependencyGraph, forceExcludes, yarnv2 = {}) {
    ...
    if (moduleVersion) {
      ...
      try {
        const modulePackagePath = path.join(
          (
            yarnv2.modulesPath ?
            path.dirname(path.resolve(process.cwd(), yarnv2.modulesPath))
            :
        ...
        if (!_.isEmpty(peerDependencies)) {
          ...
          const peerModules = getProdModules.call(this, _.map(peerDependencies, (value, key) => ({ external: key })), packagePath, dependencyGraph, forceExcludes, yarnv2);
  ...
  packExternalModules() {
    ...
    return BbPromise.try(() => Packagers.get.call(this, this.configuration.packager))
      .then(packager => {
        ...
        return packager.getProdDependencies(path.dirname(packageJsonPath), 1, this.configuration.packagerOptions)
          .then(dependencyGraph => {
            ...
            const compositeModules = _.uniq(_.flatMap(stats.stats, compileStats => {
              ...
              return getProdModules.call(this, externalModules, packagePath, dependencyGraph, packageForceExcludes, this.configuration.packagerOptions.yarnv2);
            }));
            ...
            const compositeModulePath = path.join(this.configuration.packagerOptions.yarnv2 ? '/tmp/.webpack' : this.webpackOutputPath, 'dependencies');
            ...
            const packageLockPath = path.join(path.dirname(this.configuration.packagerOptions.yarnv2 && this.configuration.packagerOptions.yarnv2.packageLockPath || packageJsonPath), packager.lockfileName);
              ...
              .mapSeries(compileStats => {
                ...
                const prodModules = getProdModules.call(this,
                  _.concat(
                    getExternalModules.call(this, compileStats),
                    _.map(packageForceIncludes, whitelistedPackage => ({ external: whitelistedPackage }))
                  ), packagePath, dependencyGraph, packageForceExcludes, this.configuration.packagerOptions.yarnv2);
