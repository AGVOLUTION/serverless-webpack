...
class Yarn {
  ...
  static getProdDependencies(cwd, depth, packagerOptions = {}) {
    ...
    let args = [ 'list', `--depth=${depth || 1}`, '--json', '--production' ];

    if (packagerOptions.yarnv2) {
      args = [ 'info', '--json' ];
    }
      ...
      .then(stdout =>
        BbPromise.try(() => {
          ...
          if (packagerOptions.yarnv2) {
            return {
              data: {
                trees: parsedLines.filter(Boolean).map(line => {
                  const name = line.value.replace('@npm:', '@');
                  return { name, children: (line.children && line.children.Dependencies && line.children.Dependencies.map(d => ({name: d.locator.replace('@npm:', '@')}))) || [] }
                })
              }
            };
          }
